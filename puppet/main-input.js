...
const paieskosFn = (raktinis,miestas,id)=>{
    return new Promise((resolve,reject)=>{
        Promise.all([cvMarket(raktinis,miestas,id),cvBankas(raktinis,miestas,id),cv(raktinis,miestas,id)]).then((result)=>{console.log(result); return resolve(result)}).catch((err)=>{console.log(err)});
